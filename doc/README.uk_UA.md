...
- **Девʼять розкладок наборів даних з однієї бази даних** - обираються створеним нащадком `LibraryContext` і реалізовані окремим класом-експортером кожна:
  - **Звичайний текст** (`PlainTxtExportLibraryContext`) - по одному файлу `<імʼя-анотації>.txt` на кожне імʼя анотації, де кожен рядок називає зображення і його прямокутники;
  - **YOLO v4 (darknet)** (`Yolo4ExportLibraryContext`) - ціла тренувальна директорія darknet для детектора YOLO v4: `data/obj.names`, `data/obj.data`, опис `cfg/yolov4-obj.cfg` цілої 162-шарової мережі YOLO v4, записаний під класи проекту, скопійовані зображення з їхніми нормалізованими файлами міток `.txt`, переліки `train.txt` і `val.txt` та порожня `backup/`;
  - **Ultralytics YOLO - детекція** (`UltralyticsDetectExportLibraryContext`) - набір даних для детекції, який читає кожен випуск Ultralytics, розкладка, що її запровадив YOLO v5, а v8, v11 і наступні за ними зберегли: єдиний дескриптор `data.yaml` над директоріями `images/train` та `labels/train`, один рядок `клас центр-x центр-y ширина висота` на прямокутник;
  - **Ultralytics YOLO - орієнтовані обмежувальні рамки** (`UltralyticsObbExportLibraryContext`) - та сама директорія і той самий дескриптор для задачі орієнтованих обмежувальних рамок, один рядок `клас x1 y1 x2 y2 x3 y3 x4 y4` із чотирьох кутів рамки на прямокутник;
  - **Ultralytics YOLO - сегментація** (`UltralyticsSegmentExportLibraryContext`) - та сама директорія і той самий дескриптор для задачі сегментації екземплярів, один рядок-багатокутник `клас x1 y1 ... xn yn` на прямокутник;
  - **COCO JSON** (`CocoExportLibraryContext`) - набір даних для детекції об'єктів COCO, який читають Detectron2, MMDetection, torchvision, трансформери детекції HuggingFace, CVAT, FiftyOne, Label Studio і Roboflow: скопійовані зображення і єдиний дескриптор `annotations/instances_default.json` над ними, що несе кожен прямокутник як пікселі `[x, y, ширина, висота]` того зображення, на якому його намалювали, замість нормалізованих чисел, яких вимагає кожна розкладка YOLO вище;
  - **Pascal VOC XML** (`PascalVocExportLibraryContext`) - набір даних Pascal VOC у формі директорій devkit, ті самі файли, які записує і читає [LabelImg](https://github.com/HumanSignal/labelImg), і те, що беруть `VOCDetection` з torchvision та `XMLDataset` з MMDetection: скопійовані зображення під `JPEGImages/`, по одному дескриптору `Annotations/<основа>.xml` на зображення над ними і переліки `ImageSets/Main`, що їх називають, де кожен прямокутник є двома кутовими точками, між якими його намалювали, а імʼя його класу записане як є, а не як індекс;
  - **Create ML JSON** (`CreateMLExportLibraryContext`) - набір даних для детекції об'єктів Create ML, шлях від проекту до детектора Core ML, що працює на iPhone: скопійовані зображення і єдиний дескриптор `annotations.json` поруч із ними в одній пласкій директорії, що є джерелом даних [`directoryWithImagesAndJsonAnnotation`](https://developer.apple.com/documentation/createml/building-an-object-detector-data-source) з `MLObjectDetector` від Apple і тією самою текою, яку бере застосунок Create ML, коли її кидають до його тренувального лотка, де кожен прямокутник несе піксельний **центр** рамки поруч із її розміром - те саме ділення навпіл, яке виконує кожна розкладка YOLO вище, лише лишене в пікселях зображення замість нормалізованого;
  - **PyTorch Vision `ImageFolder`** (`PyTorchExportLibraryContext`) - класифікаційна розкладка, яку читає набір даних `ImageFolder` з PyTorch Vision: одна директорія на імʼя анотації, що містить зображення, обрізані до прямокутників із тим іменем.
...
З обома увімкненими опціями набір містить 148 тестових випадків. Цілі `ENABLE_UNIT_TESTS` компілюються прямо із сирців проти gmock-замінників з [src/tests/mocks](/src/tests/mocks), тоді як `CTEST_Exporters` з `ENABLE_COMPONENT_TESTS` лінкує справжню спільну бібліотеку і керує нею винятково через публічні заголовки - точно як це робить проект нижче за течією.
