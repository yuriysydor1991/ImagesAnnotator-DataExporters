...
- `ImagesAnnotatorDataExporters011` - everything installed from [src/lib/facade/public](/src/lib/facade/public): `LibraryFacade`, `ILib`, `LibraryContext` with its three layout descendants, `IExporter`, `ExportContext` and the consumer implemented `IImageCropperFacility`;
- the implementation, whose symbols stay inside the shared object: `LibMain`, `LibFactory`, the three exporters, `ImageLoader` and `CURLController`.
