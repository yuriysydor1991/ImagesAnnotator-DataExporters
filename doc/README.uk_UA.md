...
- **Девʼять розкладок наборів даних з однієї бази даних** - обираються створеним нащадком `LibraryContext` і реалізовані окремим класом-експортером кожна:
  ...
  - `UltralyticsDetectExportLibraryContext` - набір даних для детекції, який читає кожен випуск Ultralytics, розкладка, що її запровадив YOLO v5, а v8, v11 і наступні за ними зберегли: єдиний дескриптор `data.yaml` над директоріями `images/train` та `labels/train`, один рядок `клас центр-x центр-y ширина висота` на прямокутник;
  - `UltralyticsObbExportLibraryContext` - та сама директорія і той самий дескриптор для задачі орієнтованих обмежувальних рамок, один рядок `клас x1 y1 x2 y2 x3 y3 x4 y4` із чотирьох кутів рамки на прямокутник;
  - `UltralyticsSegmentExportLibraryContext` - та сама директорія і той самий дескриптор для задачі сегментації екземплярів, один рядок-багатокутник `клас x1 y1 ... xn yn` на прямокутник;
  - `CocoExportLibraryContext` - набір даних для детекції об'єктів COCO, який читають Detectron2, MMDetection, torchvision, трансформери детекції HuggingFace, CVAT, FiftyOne, Label Studio і Roboflow: скопійовані зображення і єдиний дескриптор `annotations/instances_default.json` над ними, що несе кожен прямокутник як пікселі `[x, y, ширина, висота]` того зображення, на якому його намалювали, замість нормалізованих чисел, яких вимагає кожна розкладка YOLO вище;
  - `PascalVocExportLibraryContext` - набір даних Pascal VOC у формі директорій devkit, ті самі файли, які записує і читає [LabelImg](https://github.com/HumanSignal/labelImg), і те, що беруть `VOCDetection` з torchvision та `XMLDataset` з MMDetection: скопійовані зображення під `JPEGImages/`, по одному дескриптору `Annotations/<основа>.xml` на зображення над ними і переліки `ImageSets/Main`, що їх називають, де кожен прямокутник є двома кутовими точками, між якими його намалювали, а імʼя його класу записане як є, а не як індекс;
