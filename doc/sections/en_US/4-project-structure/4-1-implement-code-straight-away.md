...
A call chain through the library is short. A consumer fills the `LibraryContext` descendant of the wanted layout, hands it to `ILib::perform_export()` and gets the dataset written to `LibraryContext::export_path`:
...
- [src/lib/libmain/LibMain.cpp](/src/lib/libmain/LibMain.cpp) - the only `ILib` implementation. `LibMain::perform_export()` creates the exporter of the given context, stores it back into `ctx->exporter`, copies `export_path`, `dbProvider` and `cropper` into a fresh `ExportContext` and calls `IExporter::export_db()` on it.
...
1. Declare a new empty `LibraryContext` descendant in [src/lib/facade/public](/src/lib/facade/public), copying one of the three existing headers. Mark it `IADE_API` - the cast below needs its typeinfo to leave the shared object. This is the only installed header the change adds.
