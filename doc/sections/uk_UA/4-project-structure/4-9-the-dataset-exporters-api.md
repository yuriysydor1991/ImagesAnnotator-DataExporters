...
`CocoExportLibraryContext` і `PascalVocExportLibraryContext` - дві розкладки переліку, які читає щось інше, ніж тренувальний прохід YOLO: дескриптор COCO JSON беруть Detectron2, MMDetection, torchvision, трансформери детекції HuggingFace, CVAT, FiftyOne, Label Studio і Roboflow, а директорію devkit VOC читають `VOCDetection` з torchvision, `XMLDataset` з MMDetection і LabelImg. Обидві ж передають прямокутник недоторканим, тоді як кожна розкладка YOLO спершу ділить його числа на розмір зображення - `bbox` COCO як пікселі `[x, y, ширина, висота]` зображення, `bndbox` VOC як дві кутові точки у тих самих пікселях. `PascalVocExportLibraryContext` до того ж є однією з двох розкладок, які записують імʼя анотації, а не його позицію у впорядкованій множині імен, тож додане до проекту імʼя не перенумеровує нічого з експортованого раніше.

`CreateMLExportLibraryContext` - єдина розкладка переліку, яка взагалі виводить за межі тренувальних фреймворків: вона записує те, що читає `MLObjectDetector` від Apple, а натренована з неї модель є детектором Core ML, який працює на iPhone. Вона ж є єдиною розкладкою, чиї координати не є ані кутом, ані нормалізованими - `x`/`y` є **центром** рамки у власних пікселях зображення, тобто тим самим діленням навпіл, яке три розкладки `Ultralytics*` виконують перед діленням на розмір зображення, - і другою, поруч із `PascalVocExportLibraryContext`, яка записує імʼя класу замість індексу.

### LibraryContext

Єдиний клас даних бібліотеки, той самий, яким керуються обидві її точки входу: одноразовий `ILib::perform_export()` і `IExporter::export_db()` збудованого власноруч експортера. Створюй його методом-фабрикою `LibraryFacade` бажаної розкладки - `create_plain_txt_library_context()`, `create_yolo4_library_context()`, `create_ultralytics_detect_library_context()`, `create_ultralytics_obb_library_context()`, `create_ultralytics_segment_library_context()`, `create_coco_library_context()`, `create_pascal_voc_library_context()`, `create_createml_library_context()` чи `create_pytorch_library_context()` - або інстанціюй того нащадка самотужки, як це робить споживач, шаблонізований за типом розкладки.
...
| `create_coco_library_context()` | новий порожній `CocoExportLibraryContextPtr` |
| `create_pascal_voc_library_context()` | новий порожній `PascalVocExportLibraryContextPtr` |
...
Виклик `std::filesystem::create_directories` присутній тому, що лише експортер YOLO v4, три експортери Ultralytics YOLO, експортер COCO та експортер Pascal VOC створюють свою директорію призначення самотужки. Формати простого тексту та PyTorch Vision очікують, що `export_path` уже існує.
