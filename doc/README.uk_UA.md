...
- **Одноразова точка входу** - заповни нащадка `LibraryContext` потрібної розкладки директорією призначення і базою даних, і `ILib::libcall()` побудує потрібний експортер та запустить його. `LibraryFacade::create_exporter()` дає той самий результат із дрібнішим контролем.
